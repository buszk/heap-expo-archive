...
#define LIST_OVERFLOW_CHECK(l, i)                                              \
    if (l != root && malloc_usable_size(l) / sizeof(node) <= i)                \
    assert(false)
#else
#define LIST_OVERFLOW_CHECK(l, i)
#endif
...
#define SET_LIST_NODE_DATA(l, i, v)                                            \
    LIST_OVERFLOW_CHECK(l, i);                                                 \
    l[i].u.data = v
#define SET_LIST_NODE_LEAF(l, i, v)                                            \
    LIST_OVERFLOW_CHECK(l, i);                                                 \
    l[i].leaf = v
#define GET_LIST_NODE_DATA(r, l, i)                                            \
    LIST_OVERFLOW_CHECK(l, i);                                                 \
    r = l[i].u.data
#define GET_LIST_NODE_LEAF(r, l, i)                                            \
    LIST_OVERFLOW_CHECK(l, i);                                                 \
    r = l[i].leaf
...
#define SET_LIST_NODE_NEXT(l, i, v)                                            \
    LIST_OVERFLOW_CHECK(l, i);                                                 \
    l[i].u.next.store(v, std::memory_order_release)
#define GET_LIST_NODE_NEXT(r, l, i)                                            \
    LIST_OVERFLOW_CHECK(l, i);                                                 \
    r = l[i].u.next.load(std::memory_order_acquire)
#define INIT_LIST_NODE_NEXT(l, i, n)                                           \
    node *tmp = (node *)__calloc(sizeof(node), n);                             \
    node *null_node = nullptr;                                                 \
    if (!l[i].u.next.compare_exchange_weak(null_node, tmp))                    \
    __free(tmp)
#else
...
#define GET_LIST_NODE_NEXT(r, l, i)                                            \
    ...
    r = l[i].u.next
#define INIT_LIST_NODE_NEXT(l, i, n)                                           \
    l[i].u.next = (node *)__calloc(sizeof(node), n)
#endif

extern "C" size_t malloc_usable_size(void *ptr);

template <class T> class shadow {
    ...
    void cleanup(node *list, int level) {
        ...
        for (uint32_t i = 0; i < NUM_CHILD_LEVEL(level); i++) {
            ...
            if (LIST_NODE_LEAF(list, i))
                ...
                // i,NUM_CHILD_LEVEL(level +1), level+1);
  ...
  public:
    ...
    void insert_range_level(uintptr_t start, uintptr_t end, T *meta, node *list,
                            int level);

    bool covered(uintptr_t addr, size_t size);
};

template <class T> void shadow<T>::insert(uintptr_t addr, T *meta) {
    node *cur = root;
    uint8_t c;
...
template <class T> T *shadow<T>::find(uintptr_t addr) {
    node *cur = root;
    uint8_t c;
    for (int i = 0; i < PTR_BYTES - 1; i++) {
        GET_PTR_BYTE(c, addr, i);
...
inline uintptr_t compute_min(uintptr_t start, uint8_t mask, int level) {
    ...
    uint64_t res = start >> ((PTR_BYTES - level) * 8)
                                << ((PTR_BYTES - level) * 8);
    res += m;
...
inline uintptr_t compute_max(uintptr_t end, uint8_t mask, int level) {
    ...
    uint64_t res = end >> ((PTR_BYTES - level) * 8)
                              << ((PTR_BYTES - level) * 8);
    res += m;
...
void shadow<T>::insert_range_level(uintptr_t start, uintptr_t end, T *meta,
    ...
    for (int j = c1; j <= c2; j++) {
        ...
        if (LIST_NODE_LEAF(cur, j)) {
            assert(list != nullptr &&
                   "Overwrite a leaf node in non-bottom level");
...
// write(2, "unlock\n", 7);
        ...
        insert_range_level(std::max(start, compute_min(start, j, level)),
                           std::min(end, compute_max(end, j, level)), meta,
                           LIST_NODE_NEXT(cur, j), level + 1);
    }
}
