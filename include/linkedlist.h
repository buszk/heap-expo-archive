...
template <typename T> struct fl_node {
    uintptr_t padding_1;
    T padding_2;
    struct fl_node<T> *next;
};

template <typename T, class Alloc = std::allocator<node<T>>> class llist {
  private:
    node<T> *head;
    uint32_t list_size;
    ...
    */
    inline int is_marked_ref(node<T> *i) {
        return (int)(((uintptr_t)i) & 0x1L);
    }

    inline node<T> *unset_mark(node<T> *i) {
        uintptr_t p = (uintptr_t)i;
        p &= ~0x1L;
        return (node<T> *)p;
    }

    inline node<T> *set_mark(node<T> *i) {
        uintptr_t p = (uintptr_t)i;
        p |= 0x1L;
        return (node<T> *)p;
    }

    inline node<T> *get_unmarked_ref(node<T> *w) {
        return (node<T> *)(((uintptr_t)w) & ~0x1L);
    }

    inline node<T> *get_marked_ref(node<T> *w) {
        return (node<T> *)(((uintptr_t)w) | 0x1L);
    }
    /*
...
template <typename T> node<T>::node(T val, node<T> *_next) {
    data = val;
    next = _next;
    padding = 0;
}

template <typename T, class Alloc> llist<T, Alloc>::llist() {
...
template <typename T, class Alloc> void llist<T, Alloc>::clear() {
    // printf("Delete list method\n");
    node<T> *n = head;
    while (head) {
        head = get_unmarked_ref(head->next);
...
template <typename T, class Alloc> int llist<T, Alloc>::size() {
    return list_size;
}
 ...
 */
template <typename T, class Alloc> node<T> *llist<T, Alloc>::add(T val) {
    ...
    do {
        ...
        fl_aba_head_new = get_fl(fl_aba + 1, fl_next);
             ...
             CAS_PTR(&fl_aba_head, fl_aba_head_last, fl_aba_head_new) !=
                 fl_aba_head_last);
 ...
 */
template <typename T, class Alloc> void llist<T, Alloc>::remove(node<T> *node) {
    ...
    while (1) {
        ...
        if (CAS_PTR(&(fl_aba_head), fl_aba_head_last, fl_aba_head_new) ==
            fl_aba_head_last) {
            FAD_U32(&(list_size));
            return;
        }
 ...
 */
template <typename T, class Alloc> int llist<T, Alloc>::check() {
    node<T> *n, *unmarked;
    size_t size;
    n = head;
    size = 0;
 ...
 *  This function may ends in infinite loop if some terrible bug is present.
 */
template <typename T, class Alloc> void llist<T, Alloc>::print() {
    ...
    while (n) {
        ...
        printf("[%016lx] next: %016lx data: %08lx pad: %015lx\n", (uintptr_t)n,
               (uintptr_t)n->next, (uintptr_t)n->data, (uintptr_t)n->padding);
        n = unmarked;
    }
}
 ...
 *  This function may ends in infinite loop if some terrible bug is present.
 */
template <typename T, class Alloc> int llist<T, Alloc>::check_flist() {
    node<T> *n, *unmarked;
    size_t size;
    n = head;
    size = 0;
