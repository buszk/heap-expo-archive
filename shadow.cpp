/*
 * This is a shadow memory class that maps a memory address to a pointer
 */
...
int main() {
    uintptr_t test = 0x13371337;
    uintptr_t obj = 0xdeadbeef;
    shadow<uintptr_t> s;
    s.insert((uintptr_t)&obj, &obj);
    s.insert_range((uintptr_t)&test, sizeof(test), &test);
    printf("%lx %lx\n", s.find((uintptr_t)&test), &test);
    printf("%lx %lx\n", s.find((uintptr_t)&obj), &obj);
    printf("Passed\n");
