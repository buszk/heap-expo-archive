...
static StructType *getCtorElemTy(Module *M) {
    ...
    TyVector.push_back(
        PointerType::getUnqual(FunctionType::get(VoidTy(M), false)));
    TyVector.push_back(Int8PtrTy(M));
    ArrayRef<Type *> TyS(TyVector);
    StructType *STy = StructType::get(M->getContext(), TyS);
    return STy;
}

static ArrayType *getCtorTy(Module *M, int num) {
    ArrayType *ATy = ArrayType::get(getCtorElemTy(M), num);
    return ATy;
}
...
static void addToGlobalCtors(Module *M, Function *F) {
    ...
    if (GCL) {
        ...
        if (isa<ConstantArray>(GCL->getInitializer())) {
            ...
            ArrayType *ATy = ArrayType::get(OldCA->getType()->getElementType(),
                                            CAList.size());
            Constant *CA = ConstantArray::get(ATy, CAList);

            // If we didn't change the number of elements, don't create a new
            // GV.
    ...
    GCL = new GlobalVariable(*M, ATy, false,
                             GlobalValue::LinkageTypes::AppendingLinkage, CA,
                             "llvm.global_ctors");
}
...
struct HeapExpoGlobalTracker : public ModulePass {
    ...
    virtual bool runOnModule(Module &Mod) {
        ...
        M->getOrInsertFunction("global_hook", VoidTy(M), Int8PtrTy(M),
                               SizeTy(M));
        GlobalHookFunc = M->getFunction("global_hook");

        doInitialization(Mod);

        std::string CtorFuncName =
            "init_global_vars_" + M->getModuleIdentifier();
...
static void logVarID(int fd, DIVariable *V, uint32_t cur_call) {
    ...
    n = sprintf(buf, "%08x: %s:%d [%s]\n", cur_call, fname.c_str(),
                V->getLine(), vname.c_str());
...
struct HeapExpoFuncTracker : public FunctionPass {
    ...
    HeapExpoFuncTracker(char ID) : FunctionPass(ID) { initialized = false; }
    ...
    void instrCheck(CallInst *CI, AllocaInst *AI, DIVariable *V) {
        ...
        CastInst *cast_loc = CastInst::CreatePointerCast(AI, Int8PtrTy(M));
        std::vector<Value *> Args;
        Args.push_back(cast_loc);
        Args.push_back(CurCall);
        CallInst *checkstackvar_call =
            CallInst::Create(checkstackvar, Args, "");
    ...
    void initialize(Module *M) {
        ...
        M->getOrInsertFunction("regptr", VoidTy(M), Int8PtrTy(M), Int8PtrTy(M),
                               Int32Ty(M));
        Constant *regptr_def = M->getFunction("regptr");
        regptr = cast<Function>(regptr_def);
        regptr->setCallingConv(CallingConv::C);

        M->getOrInsertFunction("stack_regptr", VoidTy(M), Int8PtrTy(M),
                               Int8PtrTy(M), Int32Ty(M));
        ...
        M->getOrInsertFunction("checkstackvar", VoidTy(M), Int8PtrTy(M),
                               Int32Ty(M));
...
struct HeapExpoLoop : public LoopPass, CallGraphAnalysis {
    ...
    virtual bool runOnLoop(Loop *L, LPPassManager &LPM) {
        ...
        for (auto LoopItr = L->begin(), LoopItrE = L->end();
...
static RegisterPass<HeapExpoGlobalTracker> X("HeapExpoGlobal",
                                             "HeapExpo Global Pass",
                                             false /* Only looks at CFG */,
                                             false /* Analysis Pass */);
static RegisterPass<HeapExpoHeapTracker> Y("HeapExpoHeap", "HeapExpo Heap Pass",
                                           false /* Only looks at CFG */,
                                           false /* Analysis Pass */);
static RegisterPass<HeapExpoStackTracker> Z("HeapExpoStack",
                                            "HeapExpo Stack Pass",
                                            false /* Only looks at CFG */,
                                            false /* Analysis Pass */);
static RegisterPass<HeapExpoCallGraphAnalysis> W("HeapExpoCallGraph",
                                                 "HeapExpo Call Graph Analysis",
                                                 false /* Only looks at CFG */,
                                                 true /* Analysis Pass */);
