...
inline bool is_invalid(uintptr_t addr) { return ((addr & KADDR) == KADDR); }
...
inline void report_dangling(residual_pointer_t &ptr) {
    ...
    PRINTF(2,
           "[HeapExpo][Dangling]: loc[%016lx], val[%016lx], src_sig[%08x], "
...
inline void address2offset(uintptr_t *addrs, uintptr_t *offsets, int cnt) {
    for (int i = 0; i < cnt; i++) {
        Dl_info info;
        if (dladdr((void *)addrs[i], &info) && info.dli_fname != NULL &&
            info.dli_fname[0] != '\0') {
...
inline void dealloc_hook_(uintptr_t ptr, uint32_t free_sig, bool invalidate) {
    ...
    PRINTF(3, "[HeapExpo][dealloc_sig]: Object %016lx:%016lx is allocated with "
              "signature %08lx\n",
...
<<<<<<< HEAD
    ...
    for (node<uintptr_t> *it = obj->in_edges.begin(); it;
         it = obj->in_edges.iter_next(it)) {
...
>>>>>>> master
        ...
        if (invalidate)
            *(uintptr_t *)ptr_loc =
                (cur_val | KADDR | (obj->copied ? RABIT : 0) | STKBIT) - STKBIT;

        PRINTF(3, "[HeapExpo][invalidate]: ptr_loc:%016lx value:%016lx\n",
               ptr_loc, ptr_info->value);
        tmp.push_back(residual_pointer_t(
            ptr_loc, *(uintptr_t *)ptr_loc, ptr_info->src_info->signature,
            ptr_info->dst_info->signature, free_sig, ptr_info->id, counter));
...
<<<<<<< HEAD
    for (node<uintptr_t> *it = obj->out_edges.begin(); it;
         it = obj->out_edges.iter_next(it)) {
        uintptr_t ptr_loc = it->data;
=======
        ...
        for (auto &p : stack_record->list) {
            if (p.dst_info == obj && p.val == *(uintptr_t *)p.loc) {
                if (invalidate_mode > 1) {
                    *(uintptr_t *)p.loc |=
                        (KADDR | (obj->copied ? RABIT : 0) | STKBIT);
                    PRINTF(3, "[HeapExpo][invalidate_stack]: ptr_loc:%016lx "
                              "value:%016lx\n",
                           p.loc, p.val);
                }
            }
...
EXT_C void realloc_hook(char *oldptr_, char *newptr_, size_t newsize) {
    ...
    if (offset == 0 && old_info && old_info && old_info->addr == oldptr) {
        ...
        if (newsize > oldsize) {
            ...
            memory_objects->insert_range(oldptr + oldsize, newsize - oldsize,
                                         old_info);
        } else {
            /* Shrink: remove metadata */
            memory_objects->insert_range(oldptr + newsize, oldsize - newsize,
                                         nullptr);
        }
...
<<<<<<< HEAD
        for (node<uintptr_t> *it = old_info->out_edges.begin(); it;
             it = old_info->out_edges.iter_next(it)) {
            uintptr_t ptr_loc = it->data;
=======
...
>>>>>>> master
            ...
            if (ptr_loc >= oldptr + newsize) {
                PRINTF(3, "PTR[%016lx] is discarded while realloc to a smaller "
                          "area\n",
                       ptr_loc);
                if (!ptr_info->invalid) {
...
>>>>>>> master
            ...
            PRINTF(3, "MOVING %s PTR[%016lx] to %016lx, OFFSET:%016lx\n",
                   ptr_info->invalid ? "INVALID" : "", ptr_loc,
                   ptr_loc + offset, offset);
...
<<<<<<< HEAD
            ...
            auto dst_it =
                push_back_it(ptr_info->dst_info->in_edges, (ptr_loc + offset));
...
>>>>>>> master
            ...
            /* Update ptr_record */
            struct pointer_info_t *new_ptr_info =
                (struct pointer_info_t *)__malloc(
                    sizeof(struct pointer_info_t));
 ...
 */
bool get_object_addr(uintptr_t addr, uintptr_t &object_addr,
                     struct object_info_t *&object_info) {
...
>>>>>>> master
        ...
        // PRINTF(0, "src_it %016lx, val %016lx\n", src_it, ptr_loc);
        pointer_info_t pit =
            pointer_info_t(ptr_loc, ptr_val, ptr_obj_addr, obj_addr,
                           ptr_obj_info, obj_info, src_it, dst_it, id);
        struct pointer_info_t *new_ptr_info =
            (struct pointer_info_t *)__malloc(sizeof(pointer_info_t));
        *new_ptr_info = pit;
...
EXT_C void stack_regptr(char *ptr_loc_, char *ptr_val_, uint32_t id) {
    ...
    PRINTF(
        3,
        "[HeapExpo][stack_regptr]: loc:%016lx val:%016lx id:%08x obj:%016lx\n",
...
EXT_C void checkstackvar(char *ptr_loc_, uint32_t id) {
    ...
    if (ptr && ptr->dst_info->addr <= cur_val && cur_val < ptr->dst_info->addr +
    ptr->dst_info->size
    ...
    if (is_invalid(cur_val)) {
        ...
        ptr_loc,
                ptr, ptr->dst_info->addr <= cur_val, cur_val <
        ptr->dst_info->addr +
                ptr->dst_info->size, ptr->dst_info->released);
                */
        if (ptr && (ptr->dst_info->addr | KADDR) <= cur_val &&
            cur_val < (ptr->dst_info->addr + ptr->dst_info->size | KADDR) &&
            ptr->dst_info->released) {
            uint32_t store_id = ptr->store_id;
            PRINTF(2, "[HeapExpo][live_invalid_stack] PTR[%016lx] id:[%08lx] "
                      "store_id[%08lx] val[%016lx]\n",
